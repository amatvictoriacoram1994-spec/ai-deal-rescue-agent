...
const TOKEN_PATH = "credentials/google-gmail-compose-token.json";
...
async function main(): Promise<void> {
  ...
  const server = createServer((request, response) => {
    ...
    response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" }).end("Gmail Compose authorization received. You may close this tab and return to the terminal.");
    resolveCode?.(code);
  });
  ...
  console.log(`Authorize Gmail Compose access in your browser:\n${authorizationUrl.toString()}`);
  ...
  try {
    ...
    if (scopes.length !== 1 || scopes[0] !== GMAIL_COMPOSE_SCOPE) {
      throw new Error("Google returned scopes other than gmail.compose; no token was saved.");
    }
    ...
    console.log(`Success: Gmail Compose OAuth token saved to ${TOKEN_PATH}.`);
  } finally {
...
main().catch((error: unknown) => {
  console.error(`Gmail Compose authorization failed: ${error instanceof Error ? error.message : "Unknown error."}`);
  process.exitCode = 1;
});
