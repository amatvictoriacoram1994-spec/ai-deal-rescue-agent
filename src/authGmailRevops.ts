...
const CLIENT_PATH = "credentials/google-oauth-client-revops-new.json";
const TOKEN_PATH = "credentials/google-oauth-token-revops-new.json";
...
async function main(): Promise<void> {
  ...
  if (parsed.installed === undefined) throw new Error("The RevOps credential is not an installed/desktop OAuth client.");
  ...
  const server = createServer((request, response) => {
    ...
    response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" }).end("Gmail authorization received. You may close this tab and return to the terminal.");
    resolveCode?.(code);
  });
  ...
  console.log(`Authorize Gmail read-only access in your browser:\n${authorizationUrl.toString()}`);
  ...
  try {
    ...
    if (scopes.length !== 1 || scopes[0] !== GMAIL_READONLY_SCOPE) {
      throw new Error("Google returned scopes other than gmail.readonly; no token was saved.");
    }
    ...
    console.log(`Success: Gmail read-only OAuth token saved to ${TOKEN_PATH}.`);
  } finally {
...
main().catch((error: unknown) => {
  console.error(`Gmail authorization failed: ${error instanceof Error ? error.message : "Unknown error."}`);
  process.exitCode = 1;
});
